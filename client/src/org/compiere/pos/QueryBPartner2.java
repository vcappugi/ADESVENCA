...
public class QueryBPartner2 extends PosQuery
{
	/**
	 * 
	 */
	 ...
	 * 	Constructor
	 */
	public QueryBPartner2 (PosBasePanel posPanel)
	{
		super(posPanel);
	}	//	PosQueryBPartner
	...
	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(QueryBPartner2.class);
	...
	protected void init()
	{
		...
		northPanel.add(f_value, "h 30, w 200, wrap");
		...
		northPanel.add(f_name, "h 30, w 200, wrap");
		//f_name.addActionListener(this);
		
		//
		CLabel ltaxid = new CLabel(Msg.translate(p_ctx, "TaxID"));
		northPanel.add (ltaxid, " growy");
		f_taxid = new PosTextField("", p_posPanel, p_pos.getOSK_KeyLayout_ID());
		ltaxid.setLabelFor(f_taxid);
		northPanel.add(f_taxid, "h 30, w 200, wrap");
		//f_name.addActionListener(this);
		...
		northPanel.add(f_contact, "h 30, w 200 , wrap");
		...
		northPanel.add(f_email, "h 30, w 200, wrap");
		...
		CLabel ladd1 = new CLabel(Msg.translate(p_ctx, "Address1"));
		northPanel.add (ladd1, " growy");
		f_add1 = new VLocation ("C_Location_ID", false, false, true, new MLocationLookup (Env.getCtx(), 0));
		ladd1.setLabelFor(f_add1);
		northPanel.add(f_add1, "h 30, w 200, wrap");
		//f_city.addActionListener(this);
		//
		/*CLabel ladd2 = new CLabel(Msg.translate(p_ctx, "Address2"));
		northPanel.add (ladd2, " growy");
		f_add2 = new CTextField(50);
		ladd2.setLabelFor(f_add2);
		northPanel.add(f_add2, "h 30, w 200, wrap");
		//
		...
		northPanel.add(f_city, "h 30, w 200, wrap");*/
		//f_city.addActionListener(this);
	...
	public void actionPerformed (ActionEvent e)
	{
		...
		else if ("Ok".equals(e.getActionCommand()))
		{
			...
			MBPartner m_partner = MBPartner.getTemplate(Env.getCtx(), AD_Client_ID);
			...
			String value = f_value.getText();
			/*if (value == null || value.length() == 0)
			{
				//	get Table Documet No
				value = DB.getDocumentNo (Env.getAD_Client_ID(Env.getCtx()), "C_BPartner", null, m_partner);
				f_value.setText(value);
			}*/
			m_partner.setValue(f_value.getText());
			m_partner.setTaxID(f_taxid.getText());
			//
			m_partner.setName(f_name.getText());
			if (m_partner.save())
			{
				log.fine("C_BPartner_ID=" + m_partner.getC_BPartner_ID());
				m_C_BPartner_ID= m_partner.get_ID();
			}
			MBPartnerLocation m_pLocation = new MBPartnerLocation(m_partner);
			m_pLocation.setC_Location_ID(f_add1.getC_Location_ID());
			//
			m_pLocation.setPhone(f_phone.getText());
...
//			***** Business Partner - User *****
			...
			{
				if (contact.length() == 0)
					contact = f_name.getText();
				m_user.setName(contact);
				m_user.setEMail(email);
				
				
				//
				m_user.setPhone(f_phone.getText());
