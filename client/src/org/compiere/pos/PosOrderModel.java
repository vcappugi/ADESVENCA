...
public class PosOrderModel extends MOrder {
	...
	public static PosOrderModel createOrder(MPOS pos, MBPartner partner) {
		...
		order.setM_Warehouse_ID(pos.getM_Warehouse_ID());  // commerso
		...
		{
			order = null;
			throw new AdempierePOSException("Fallo La Orden");
		}
 ...
 *         *Copyright � ConSerTi
	...
	public boolean deleteOrder () {
		if (getDocStatus().equals("DR"))  // una pregunta puedes borrar cualquier orden no importa que este completada ??
			{
	...
	public boolean payCash(BigDecimal amt, int C_BankStatement_ID) {
		...
		if ( payment.processIt(MPayment.DOCACTION_Complete) )
		{
			...
			bsl.setStatementLineDate(payment.getCreated());
			bsl.setPayment(payment);
			
			bsl.setTrxAmt(amt);
			bsl.setStmtAmt(amt);
			bsl.setC_Currency_ID(205); 
			bsl.setEftTrxID(payment.getTenderType());
	...
	public boolean payCheck(BigDecimal amt, String accountNo, String routingNo, String checkNo, int C_BankStatement_ID) 
	{
	...
	public boolean payCreditCard(BigDecimal amt, String accountName, int month, int year,
			String cardNo, String cvc, String cardtype, int C_BankStatement_ID) 
	{
	...
	public boolean payGifCard(BigDecimal amt, String accountName,String cardNo, int C_BankStatement_ID) 
	{

		MPayment payment = createPayment("G");
		payment.setAmount(getC_Currency_ID(), amt);
		payment.setC_BankAccount_ID(m_pos.getC_BankAccount_ID());
	...
	public boolean payStoreCoupon(BigDecimal amt, String accountName,String cardNo) 
	{

		MPayment payment = createPayment("S");
		payment.setAmount(getC_Currency_ID(), amt);
		payment.setC_BankAccount_ID(m_pos.getC_BankAccount_ID());
		payment.setCreditCardNumber(cardNo);
		payment.setA_Name(accountName);
	...
	public boolean payVendorCoupon(BigDecimal amt, String accountName,String cardNo) 
	{

		MPayment payment = createPayment("V");
		payment.setAmount(getC_Currency_ID(), amt);
		payment.setC_BankAccount_ID(m_pos.getC_BankAccount_ID());
		payment.setCreditCardNumber(cardNo);
		payment.setA_Name(accountName);
	...
	{
		return this.m_pos.get_ValueAsBoolean("iswic");
	}
} // PosOrderModel.class
