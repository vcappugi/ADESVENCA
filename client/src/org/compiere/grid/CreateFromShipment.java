...
public class CreateFromShipment extends CreateFrom 
{
	...
	protected ArrayList<KeyNamePair> loadInvoiceData (int C_BPartner_ID)
	{
		...
		.append(" FROM C_Invoice i "
				+ "WHERE i.C_BPartner_ID=? AND i.IsSOTrx='" + getIsSOTrx()
				...
				+ " WHERE i2.C_BPartner_ID=? AND i2.IsSOTrx='" +getIsSOTrx()
				+ "' AND i2.DocStatus IN ('CL','CO') "
		...
		for (int i = 0; i < miniTable.getRowCount(); i++)
		{
			if (((Boolean)miniTable.getValueAt(i, 0)).booleanValue()) {
				...
				if (C_OrderLine_ID != 0)
				{
					...
					iol.setM_Product_Cost(ol.getLineNetAmt());
					iol.setM_Product_Cost(ol.getPriceEntered());
				}
				else if (il != null)
				{
