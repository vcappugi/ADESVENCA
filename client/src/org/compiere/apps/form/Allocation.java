...
//		paymentTable.setColumnClass(i++, BigDecimal.class, true);      	//  8-Multiplier
	...
	public Vector<Vector<Object>> getInvoiceData(boolean isMultiCurrency, Object date, IMiniTable invoiceTable)
	{
		...
		StringBuffer sql = new StringBuffer("SELECT i.DateInvoiced,i.DocumentNo,i.C_Invoice_ID," //  1..3
			...
			+ "i.MultiplierAP, "
			+ "(SELECT coalesce(SUM(TaxAmt),null,0)*i.MultiplierAP FROM LCO_InvoiceWithholding where C_Invoice_ID =i.C_Invoice_ID and Iscalconpayment='Y' AND Processed='N' AND IsActive='Y'), i.Num_Factura " //10 withholding, 11 Num Factura
		...
		try
		{
			...
			while (rs.next())
			{
				...
				line.add(rs.getBigDecimal(10));      			//  6/8-WriteOff
...
//				line.add(rs.getBigDecimal(9));		//	8/10-Multiplier
				...
				if (Env.ZERO.compareTo(open) != 0)
					data.add(line);
				line.add(rs.getString(11));      			//  11 Num_factura
			}
			rs.close();
			pstmt.close();
		}
	...
	public Vector<String> getInvoiceColumnNames(boolean isMultiCurrency)
	{
		...
		// Modif vcappugi
		columnNames.add(Msg.getMsg(Env.getCtx(), "Retención"));
		//
		columnNames.add(Msg.getMsg(Env.getCtx(), "AppliedAmt"));
		columnNames.add(Msg.getMsg(Env.getCtx(), "OverUnderAmt"));
		columnNames.add(Msg.getMsg(Env.getCtx(), "Num Factura"));
//		columnNames.add(" ");	//	Multiplier
	...
	public void setInvoiceColumnClass(IMiniTable invoiceTable, boolean isMultiCurrency)
	{
		...
		invoiceTable.setColumnClass(i++, String.class, true);           // 11-Num Factura
	...
	public String writeOff(int row, int col, boolean isInvoice, IMiniTable payment, IMiniTable invoice, boolean isAutoWriteOff)
	{
		...
		else 
		{
			...
			if (col == 0)  //selection
			{
				...
				{
					applied = open;    //  Open Amount
					applied = applied.subtract(discount).subtract(writeOff);
					...
					if ( isAutoWriteOff )
						...
						overUnder = open.subtract(applied.add(discount).add(writeOff));
				}
				else    //  de-selected
				{
					//writeOff = Env.ZERO;
	...
	public String saveData(int m_WindowNo, Object date, IMiniTable payment, IMiniTable invoice, String trxName)
	{
		...
		for (int i = 0; i < iRows; i++)
		{
			...
			if (((Boolean)invoice.getValueAt(i, 0)).booleanValue())
			{
				...
				//Ajuste para cuadre de reteciones al momento de asignacion automática
				BigDecimal WriteOffAmt = (BigDecimal)invoice.getValueAt(i, i_writeOff);
				//BigDecimal WriteOffAmt = DB.getSQLValueBD(null, "SELECT coalesce(SUM(TaxAmt),null,0)*-1 FROM LCO_InvoiceWithholding where C_Invoice_ID ="+ C_Invoice_ID +" and Iscalconpayment='Y'");
				
				//AppliedAmt = AppliedAmt.subtract(WriteOffAmt);
