...
*/
	...
	private boolean defaultsOK ()
	{
		...
		checkVersionclient();   // Verificar la Version del cliente rmontano
	...
	// Chequea la version del cliente contra la base de dato
	private boolean checkVersionclient(){
		String versionclit = Adempiere.COPYRIGHT;
		String version = DB.getSQLValueString(null,"Select substring(lastbuildinfo, position('}' in lastbuildinfo) + 2, 8) From AD_System");
		int i = versionclit.indexOf(":") + 1;
		versionclit = DB.getSQLValueString(null,"Select to_char(to_date('"+versionclit.substring(i, (i + 8))+"','ddMMyyyy'),'yyyyMMdd')");
		
		//Comparar las versiones de base de dato y la aplicacion
		if(Integer.valueOf(versionclit) < Integer.valueOf(version)){
			StringBuffer msg = new StringBuffer (">>\n");
			msg.append(res.getString("VersionConflict")).append("\n")
				.append(res.getString("VersionInfo")).append("\n");
			msg.append(version == null ? "null" : version).append(" <> ")
				.append(versionclit).append("\n");
			msg.append("Por favor Actualizar su Cliente para continuar...").append("\n<<");
			JOptionPane.showMessageDialog(null, msg.toString(),
				Adempiere.getName() + " - " + res.getString("VersionConflict"),
				JOptionPane.ERROR_MESSAGE);
			AEnv.exit(1);
