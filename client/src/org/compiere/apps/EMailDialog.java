	...
	implements ActionListener, VetoableChangeListener
{
		...
		String subject, String message, File attachment)
	{
		...
		//To Pre-Load AD_User
        if(Env.getContextAsInt(Env.getCtx(), "#C_BPartner_Email_ID")>0)
        {
        	String sql ="Select MAX(AD_User_ID) from AD_User WHERE IsActive='Y' AND C_BPartner_ID="+Env.getContextAsInt(Env.getCtx(), "#C_BPartner_Email_ID");
        	int AD_User_ID = DB.getSQLValue(null, sql);
        	MUser user = null;
        	if(AD_User_ID>0)
        	{
        		user = new MUser(Env.getCtx(), AD_User_ID, null);
        		fUser.setValue(user.getAD_User_ID());
        		to=user.getEMail();
        	}
        } //Pre-Load
	...
	public void setFrom(MUser newFrom)
	{
		...
		MClient cliente= new MClient (Env.getCtx(),Env.getAD_Client_ID(Env.getCtx()),null);
		m_from.setEMailUser(cliente.getRequestUser());
		m_from.setEMail(cliente.getRequestUser());
		m_from.setEMailUserPW(cliente.getRequestUserPW());
