...
import org.pentanet.model.MHRequest;
	...
	implements FormPanel, ActionListener, ListSelectionListener
{
	...
	DefaultTableModel 	selTableModel = new DefaultTableModel(
			new String[]{"ID",
				...
				Msg.translate(Env.getCtx(), "Applicant"),
				Msg.translate(Env.getCtx(), "DateDoc"),
				Msg.translate(Env.getCtx(), "Summary"),
				Msg.translate(Env.getCtx(), "Total")}, 0); 
	...
	private CButton bZoom = AEnv.getButton("Zoom24");
	...
	private void jbInit () throws Exception
	{
		...
		selTable.setColumnClass(3, String.class, true);        //  3-AD_Org_ID
		selTable.setColumnClass(4, String.class, true);        //  4-Applicant
		selTable.setColumnClass(5, Timestamp.class, true);     //  6-DateDoc
		selTable.setColumnClass(6, String.class, true);        //  6-Summary
		selTable.setColumnClass(7, String.class, true);        //  7-Amount (optional)
	...
	//	answers.setOpaque(false);
		...
		//
		int row = 0;
		selPane.setPreferredSize(new Dimension(width, 200));
		selPane.setMinimumSize(new Dimension(100, 200));
		...
		buttonPanel.add(bZoom);
		buttonPanel.add(lTextRell);
		buttonPanel.add(bCancel);
		buttonPanel.add(bOK);
		
		centerPanel.add (buttonPanel, new GridBagConstraints (3, row, 1, 1, 0.0, 0.0, 
				GridBagConstraints.EAST, GridBagConstraints.NONE, 
	...
	public int loadActivities()
	{
		...
		try
		{
			...
			while (rs.next ())
			{
				...
				String sqlu = "SELECT u.description FROM AD_WF_Process p "
						+ " INNER JOIN AD_User u ON u.AD_User_ID = p.AD_User_ID "
						+ " WHERE AD_WF_Process_ID = " + activity.getAD_WF_Process_ID();
				MTable table = new MTable(Env.getCtx(), activity.getAD_Table_ID(), null);
				String sqlo = "SELECT name FROM AD_Org WHERE AD_Org_ID IN "
						+ " (SELECT AD_Org_ID FROM "+ table.getTableName() +" WHERE " + table.getTableName() + "_ID =" + activity.getRecord_ID() +")" ;
				
				Object[] rowData = new Object[8];
				rowData[0] = new IDColumn(activity.get_ID());
				rowData[1] = activity.getPriority();
				rowData[2] = activity.getNodeName();
				rowData[3] = DB.getSQLValueString(null, sqlo);
				rowData[4] = DB.getSQLValueString(null, sqlu);
				

				if (table.getTableName().equals("M_Requisition")){
					MRequisition rqdatedoc = new MRequisition(Env.getCtx(), activity.getRecord_ID(), null);
					rowData[5] = rqdatedoc.getDateRequired();
				}else if (table.getTableName().equals("C_Order")){
					MOrder Ordatedoc = new MOrder(Env.getCtx(), activity.getRecord_ID(), null);
					rowData[5] = Ordatedoc.getDateOrdered();
				}else if (table.getTableName().equals("H_Request")){
					MHRequest Rtdatedoc = new MHRequest(Env.getCtx(), activity.getRecord_ID(), null);
					rowData[5] = Rtdatedoc.getRequestDate();
				}else{
					String sqlC = "SELECT COUNT(*) FROM AD_Column c INNER JOIN AD_Table t ON t.AD_Table_ID = c.AD_Table_ID " +
								  "WHERE c.ColumnName = 'DateAcct' AND t.TableName='" + table.getTableName() + "'";
					
					String sqld = "SELECT now()";
					if(DB.getSQLValue(null, sqlC)>0)
						sqld ="SELECT DateAcct FROM "+ table.getTableName() +" WHERE " + table.getTableName() + "_ID =" + activity.getRecord_ID();
					rowData[5] = DB.getSQLValueTS(null, sqld);
				}
				
				rowData[6] = activity.getSummary();
				rowData[7] = totalstring(activity.get_ID()) ;
	...
	public void display(IDColumn id)
	{
		...
		if (MWFNode.ACTION_UserChoice.equals(node.getAction()))
		{
			...
			if (m_column != null && m_column.get_ID() != 0)
			{
				...
				{
					ValueNamePair[] values = MRefList.getList(Env.getCtx(), 319, true);		//	_YesNo
	...
	public void actionPerformed (ActionEvent e)
	{
		...
		if (e.getSource() == bZoom)
			...
			if (fAnswerList.getItemCount() == 3)
				fAnswerList.setSelectedIndex(2);
			else
	...
	private void cmd_OK()
	{
		...
		//	Next
		// loadActivities();
		selTableModel.removeRow(selTable.getSelectedRow());	
		resetDisplay(null);
		
	}	//	cmd_OK
	/**
	 * Message to user required
	 */
	...
	private String totalstring (int id) {
		String sql="";
		String resultado="";
		int table = DB.getSQLValue(null,"Select AD_Table_ID From AD_WF_Activity where AD_WF_Activity_ID=" + id);
		int req =DB.getSQLValue(null,"Select Record_ID From AD_WF_Activity where AD_WF_Activity_ID=" + id);
		if (table==702)
			sql = "SELECT to_char(TotalLines,'999,999,999.99') from M_Requisition where M_Requisition_ID=" + req;
		else if (table==259)
			sql = "SELECT to_char(GrandTotal,'999,999,999.99') from C_Order where C_Order_ID=" + req;
		else if (table==318)
			sql = "SELECT to_char(GrandTotal,'999,999,999.99') from C_Invoice where C_Invoice_ID=" + req;
		else
			resultado ="-";
		
		if (resultado !="-")
			resultado = DB.getSQLValueString(null, sql);
