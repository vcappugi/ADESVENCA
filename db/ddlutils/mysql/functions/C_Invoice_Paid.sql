 ...
 * 
 * Test..........: SELECT invoicePaid( ??? ); => 
 *                 SELECT C_Invoice_ID, IsPaid, IsSOTrx, GrandTotal, invoicePaid (C_Invoice_ID, C_Currency_ID, MultiplierAP) FROM C_Invoice_v ORDER BY C_Invoice_ID;
 * +--------------+--------+---------+-----------------+---------------------------------------------------------+
 * | C_Invoice_ID | IsPaid | IsSOTrx | GrandTotal      | invoicePaid (C_Invoice_ID, C_Currency_ID, MultiplierAP) |
 * +--------------+--------+---------+-----------------+---------------------------------------------------------+
...
-- ## Drop statement
DROP FUNCTION IF EXISTS invoicePaid;

DELIMITER $$
-- ## Create statement
CREATE FUNCTION invoicePaid (
