...
<table class="main">
	<html:form action="/CreateGoodsReceiveAction" focus="qtyAndItem">
	...
	<tr>
   		...
   		<div id="searchPanel">
			...
			<tr>
				<td>
					<label><pos:message key="vendor.ref"/>:</label>	
				</td>
	...
	<tr>
		<td colspan="2" align="right">
			<html:button property="btn" accesskey="c" styleClass="continue smallbutton" onclick="createGoodReceiveNote()">&nbsp;</html:button>
		</td>
	</tr>
	<tr>					  
		<td align="center">
...
function createGoodReceiveNote()
{
	var method = "createPOSGoodsReceiveNote";
	var myForm = document.forms[0];
	var actionElement = document.getElementsByName('action')[0];
...
document.forms[0].barCode.onkeyup = function(e){
	...
	var orderType = '<%=Constants.GOODS_RECEIVE_NOTE%>';
	...
	function incrementCart(productId)
	{
		try
		{
			...
			var pars = 'action=incrementQty&productId='+productId+ '&ifAdd=true'+ '&isSales=false' + '&orderType=' + '<%=Constants.GOODS_RECEIVE_NOTE%>';
	...
	function decrementCart(productId)
	{
		...
		{
			var url = 'CreateGoodsReceiveAction.do';
			var pars = 'action=decrementQty&productId='+productId+ '&ifAdd=false'+ '&isSales=false' + '&orderType=' + '<%=Constants.GOODS_RECEIVE_NOTE%>';;
	...
	function deleteItemFromCart(productId)
	{
		try
		{
			...
			var pars = 'action=deleteFromPOSGRNCart&productId='+productId + '&orderType=' + '<%=Constants.GOODS_RECEIVE_NOTE%>';;
