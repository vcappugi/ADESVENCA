...
public class MHRequest extends X_H_Request implements DocAction, DocOptions{
	 ...
	 */
	public MHRequest (Properties ctx, int H_Request_ID, String trxName)
	{
	 ...
	 */
	public MHRequest (Properties ctx, ResultSet rs, String trxName)
	{
	...
	{
		setH_Request_ID(0);		//	forces new Record
	...
	public void setProcessed (boolean processed)
	{
		...
		String set = "SET Processed='"
			+ (processed ? "Y" : "N")
			+ "' WHERE H_Request_ID=" + getH_Request_ID();
	...
	public boolean voidIt() {
		...
		String tableName = DB.getSQLValueString(null, "SELECT tablename FROM AD_Table WHERE AD_Table_ID = ?", getAD_Table_ID());
		String sql = "SELECT COUNT(*) FROM " + tableName + " WHERE DocStatus IN ('IP','CO','CL') AND " + tableName + "_ID = " + getRecord_ID();
		
		if(tableName==null)
			return false;
			
		if(DB.getSQLValue(null, sql)>0){
			m_processMsg = "No se puede Anular. ***Documento Relacionado***";
			return false;
		}
	...
	protected boolean beforeDelete() {
		...
		String valida =  DB.getSQLValueString(null,"SELECT DocStatus FROM H_Request WHERE H_Request_ID=" + getH_Request_ID());
