	...
	public static MHRPayrollConcept[] getPayrollConcepts (MHRProcess p)
	{
		...
		if (p.getHR_Payroll_Type_Line_ID() != 0 ){
				wherel = " AND (CASE WHEN not HR_Payroll_Type_line_ID is null THEN"
						...
						+ "(HR_Payroll_Type_line_ID = "+p.getHR_Payroll_Type_Line_ID()+" OR HR_Payroll_Type_line2_ID = "+p.getHR_Payroll_Type_Line_ID()+")"
						+ "	ELSE HR_Payroll_Type_line_ID = "+p.getHR_Payroll_Type_Line_ID()+" END)"
						+ "ELSE 1=1 END)";
		}
		
		List<MHRPayrollConcept> list = new Query(p.getCtx(), Table_Name, COLUMNNAME_HR_Payroll_ID+"=?" + wherel , null)
