	...
	implements DocAction, ProcessCall
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if(newDoc == true)
		{
			...
			int  AD_Sequence_Org_ID = DB.getSQLValue(null, sql, getC_DocType_ID(),getAD_Org_ID());
		...
		if(!isReceipt() && getTenderType().equals("A") && getCheckNo()==null)
		{
			String sql = "SELECT AD_Sequence_Org_ID FROM AD_Sequence_Org WHERE IsActive='Y' AND IsDocumentNo='N' AND C_DocType_ID=? AND AD_Org_ID=?";
			int  AD_Sequence_Org_ID = DB.getSQLValue(null, sql, getC_DocType_ID(),getAD_Org_ID());
			
			if(AD_Sequence_Org_ID>0)
			{
				X_AD_Sequence_Org seq = new X_AD_Sequence_Org(getCtx(), AD_Sequence_Org_ID, get_TrxName());
				setCheckNo(seq.getCurrentNext()+"");
				seq.setCurrentNext(seq.getCurrentNext() + seq.getIncrementNo());
				seq.save();
			}
		}
