...
public class MOrder extends X_C_Order implements DocAction,DocOptions
{
	...
	public String prepareIt()
	{
		...
		setDateAcct(getDateAct()); // Cambiar fecha contable
		//	Std Period open?
			...
			if (product != null) 
			{
				if (product.isStocked())
				{
					...
					if (!MStorage.add(getCtx(), line.getM_Warehouse_ID(), M_Locator_ID, 
						...
						//Por Vcappugi
					{
						MStorage mst = MStorage.get(getCtx(), M_Locator_ID, line.getM_Product_ID(), line.getM_AttributeSetInstance_ID(), get_TrxName());
						mst.setM_Product_Cost(line.getPriceEntered());
						//mst.setAD_Table_ID(259);
						//mst.setRecord_ID(line.getC_Order_ID());
						mst.save();
	...
	public boolean voidIt()
	{
		...
		//Verifica que no tenga documentos atados
		if(!verifyDoc())
			return false;
		...
		int executeUpdate = DB.executeUpdate("delete from C_BudgetPublic_Exec_Line " +
				"where c_budgetpublic_exec_id in (select c_budgetpublic_exec_id from c_budgetpublic_exec where  c_order_id ="+ getC_Order_ID()+")");
	...
	public boolean closeIt()
	{
		...
		//lhernnadeza
		String sql = "SELECT dif FROM Order_Close_Verify("+ getC_Order_ID() + ")";
		//log.warning("C_Order_ID = " + getC_Order_ID());
		PreparedStatement pstmtl=null;
		ResultSet rsl=null;
		boolean cerrar=false;
		//int factt=0; 	//int qty=0;
		try
		{
			pstmtl = DB.prepareStatement (sql, get_TrxName());
			rsl = pstmtl.executeQuery();
			while (rsl.next ())
			{
				int dif=rsl.getInt(1);
				cerrar = (dif == 0) ? true : false;
			} //while
		} //try
		catch(Exception e)
		{
		...
		if (cerrar == true) //para cerrar orden
		{
		...
		for (int i = 0; i < lines.length; i++)
		{
			MOrderLine line = lines[i];
			BigDecimal old = line.getQtyOrdered();
			BigDecimal oldAmt = line.getLineNetAmt();
			boolean alm;
			
			X_M_Product pro = new X_M_Product(Env.getCtx(), line.getM_Product_ID(), get_TrxName());
			alm=pro.isStocked();
			
			if (old.compareTo(line.getQtyDelivered()) != 0 )
			{
								
				if (alm == true)//isStocked = Y
				{
					line.setQtyLostSales(line.getQtyOrdered().subtract(line.getQtyDelivered()));
					line.setQtyOrdered(line.getQtyDelivered());					
				}else {
					line.setQtyLostSales(line.getQtyOrdered().subtract(line.getQtyInvoiced()));
					line.setQtyOrdered(line.getQtyInvoiced());					
				}
				//	QtyEntered unchanged
				line.addDescription("Close (" + old + "), Amount (" + oldAmt + ")");
				line.save(get_TrxName());
			}
		}
		//	Clear Reservations
		...
		//Contabilizar Cantidades No Entregadas
		MDocType doct = new MDocType(getCtx(), getC_DocType_ID(), get_TrxName());
		if(doct.isFactAcct())
		{
			...
			if(!createFactsRev(as, 259, getC_Order_ID(), "CL"))
			{
				return false;
			}
		}
		...
		setDocAction(DOCACTION_None);
			...
			ADialog.warn(0,null, "Orden Totalmente Facturada o Existen Diferencias Entre lo Facturado y lo que se Espera en Almacen");
			return false;
		}
	...
	public boolean reverseCorrectIt()
	{
		...
		if(doct.isFactAcct())
		{
			...
			if(!createFactsRev(as, 259, getC_Order_ID(), "RE"))
			{
				return false;
			}
		}
	...
	public boolean reActivateIt()
	{
		...
		for(final MOrderLine ol: getLines())
		{
			...
			if(ol.getQtyInvoiced().signum()==0 && ol.getQtyDelivered().signum()==0){
				ol.setQtyReserved(ol.getQtyInvoiced());
				ol.setQtyOrdered(ol.getQtyEntered());
				ol.save();
			}
		}
		return true;
    ...
    /*
	 * Contabilización de Reversos (Al reversar documento)
	 */
	@SuppressWarnings("deprecation")
	public boolean createFactsRev(MAcctSchema as, int AD_Table_ID, int Record_ID, String Type)
	{
		if(getDateReverse()==null){
			m_processMsg = "Debe colocar Fecha de Reverso";
			return false;
		}
		...
		MOrder orden = new MOrder(getCtx(), Record_ID, get_TrxName());
		
		int sdn = orden.getC_BPartner_ID();
		String fecha_acct = DB.getSQLValueString(null,"SELECT to_char(DateReverse, 'dd/mm/yyyy') FROM C_Order where C_Order_id =" + Record_ID);
		int period_id = DB.getSQLValue(null, "select c_period_id from c_period where startdate <= '"+fecha_acct+"'  and enddate >= '"+fecha_acct+"'");
		
		MPeriod per = new MPeriod(getCtx(), period_id, get_TrxName());
		if(!per.isOpen("POO")){
			m_processMsg = "Periodo del Reverso Cerrado";
			return false;
		}
		...
		String sql = "SELECT OL.M_PRODUCT_ID,currencyConvert(OL.LINENETAMT,OL.C_Currency_ID,"+moneda+",O.DateAcct,null,1000000,null), "
					...
					+ "OL.C_Currency_ID, ((CASE WHEN OL.QtyDelivered >= 0 THEN OL.QtyLostSales ELSE (OL.QtyOrdered-OL.QtyInvoiced) END) * OL.PriceEntered)::numeric(10,2), "
					+ "currencyConvert((CASE WHEN OL.QtyDelivered >= 0 THEN OL.QtyLostSales ELSE (OL.QtyOrdered-OL.QtyInvoiced) END) * OL.PriceEntered, "
					+ "OL.C_Currency_ID,"+moneda+",O.DateAcct,null,1000000,null), OL.C_OrderLine_ID as line_ID "
		...
		try
		{
			...
			while (rsl.next ())
			{
				...
				//Reversado O (Cerrado y monto mayor de cero)
				if((Type.contains("RE")) || ((Type.contains("CL")) && (rsl.getBigDecimal(12).abs().compareTo(Env.ZERO)>0)))
				{
					//DEBE
					MFactAcct cont2 = new MFactAcct(getCtx(), 0, get_TrxName());  //creo un registro en fact_acct
					cont2.setC_Currency_ID(rsl.getInt(10));
					cont2.setAD_Org_ID(orden.getAD_Org_ID());
					cont2.setAD_Table_ID(AD_Table_ID);
					cont2.setC_BPartner_ID(sdn);
					cont2.setAccount_ID(account2);
					
					if(Type.contains("RE")) {
						cont2.setAmtAcctDr(rsl.getBigDecimal(2));
						cont2.setAmtSourceDr(rsl.getBigDecimal(8));
					}else {
					//	if ( rsl.getBigDecimal(12).signum() < 0){//si es negativo
					//	cont2.setAmtAcctCr(rsl.getBigDecimal(12).abs());
					//	cont2.setAmtSourceCr(rsl.getBigDecimal(11).abs());
					//	}else{
						cont2.setAmtAcctDr(rsl.getBigDecimal(12).abs());
						cont2.setAmtSourceDr(rsl.getBigDecimal(11).abs());					
					//	}
					}	
					...
					cont2.setRecord_ID(Record_ID);   
					cont2.setC_Period_ID(period_id);
					cont2.setPostingType("A");
					cont2.setM_Product_ID(rsl.getInt(1));
					cont2.setLine_ID(rsl.getInt(13));
					cont2.save();
						
					//HABER
					MFactAcct cont3 = new MFactAcct(getCtx(), 0, get_TrxName());  //creo un registro en fact_acct
					cont3.setC_Currency_ID(rsl.getInt(10));
					cont3.setAD_Table_ID(AD_Table_ID);
					cont3.setC_BPartner_ID(sdn);
					cont3.setAD_Org_ID(orden.getAD_Org_ID());
					cont3.setAccount_ID(account1);
					
					if(Type.contains("RE")) {
						cont3.setAmtAcctCr(rsl.getBigDecimal(2));
						cont3.setAmtSourceCr(rsl.getBigDecimal(8));
					}else {
					//	if ( rsl.getBigDecimal(12).signum() < 0){//si es negativo
					//		cont3.setAmtAcctDr(rsl.getBigDecimal(12).abs());
					//		cont3.setAmtSourceDr(rsl.getBigDecimal(11).abs());					
					//		}else{
							cont3.setAmtAcctCr(rsl.getBigDecimal(12).abs());
							cont3.setAmtSourceCr(rsl.getBigDecimal(11).abs());
					//		}
					}
	...
	private boolean verifyDoc(){
		...
		String sql = "SELECT COUNT(*) FROM M_MatchPO mpo " +
				"LEFT JOIN C_InvoiceLine il ON il.C_InvoiceLine_ID = mpo.C_InvoiceLine_ID " +
				...
				"LEFT JOIN M_InOut io ON io.M_InOut_ID = iol.M_InOut_ID AND io.DocStatus IN ('CO','CL')" + 
				"LEFT JOIN C_OrderLine ol ON ol.C_OrderLine_ID = mpo.C_OrderLine_ID " +
				"WHERE ol.C_Order_ID = ? ";

	//	log.warning("C_Invoice_ID = " + getC_Invoice_ID());
	//	log.warning("DB = " + DB.getSQLValue(get_TrxName(), sql, getC_Invoice_ID()));
		
		
		if(DB.getSQLValue(get_TrxName(), sql, get_ID()) > 0){
			m_processMsg = "No se puede Reversar la Orden ya que posee Otros Documentos Relacionados";
			return false;
		}
