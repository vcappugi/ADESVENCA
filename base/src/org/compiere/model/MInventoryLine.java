	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (newRecord && m_isManualEntry)
		{
			...
			if (getM_AttributeSetInstance_ID() == 0)
			{
				...
				//Unit Cost
				BigDecimal costs = getM_Product_Cost();
				BigDecimal costo_ant = DB.getSQLValueBD(null, "SELECT Coalesce(sum(M_Product_Cost*qty),0) FROM M_InventoryLine where IsResult ='N' AND Isin ='N' AND M_Inventory_ID="+ getM_Inventory_ID());
				Boolean isRes = isResult();   
				Boolean isin = isIn(); 
				...
				if (isin != null) {
				   if (isin){ //Es entrada
					   if(getQty().signum()!=0)
						   costs = costo_ant.divide(((BigDecimal) getQty().abs()),2);
					   if(getQty().signum()>0)
						   setQty(getQty().negate());
				   }else if (isRes){ //No es entrada y es Resultante 
					   costs = new BigDecimal(0);
				   }
				}   
				
				if((isin)&&!(isRes))
					costs = getM_Product_Cost();
				
				setM_Product_Cost(costs);
		...
		*/
		...
		if(getC_ProjectLine_ID() == 0){
			MInventory inv = new MInventory(Env.getCtx(), getM_Inventory_ID(), get_TrxName());
			
			String sql = "SELECT MAX(C_ProjectLine_ID) FROM C_ProjectLine WHERE M_Product_ID IN "
					+ "(SELECT M_ProductTo_ID FROM M_Product_SalesRelation WHERE M_Product_ID = ? ) "
					+ "AND C_Project_ID = ?";
			int project = DB.getSQLValue(null, sql, getM_Product_ID(), inv.getC_Project_ID());
			if(project!=0){
				setC_ProjectLine_ID(project);
			}
