...
public class MRequisition extends X_M_Requisition implements DocAction, DocOptions
{
	...
	//Solo se borra lo que este en estatus borrador
	@Override
	protected boolean beforeDelete() {
		Boolean res;
		String valida =  DB.getSQLValueString(null,"SELECT DocStatus FROM M_Requisition WHERE M_Requisition_ID=" + getM_Requisition_ID());
	...
	public String prepareIt()
	{
		...
		setDateDoc(getDateAct()); // Cambiar fecha contable
		//	Std Period open?
	...
	public String completeIt()
	{
		...
		//Actualiza el plan de compras 
		Compra(getM_Requisition_ID());
		//
	...
	public String Compra(int M_Requisition_ID){
		String sql = "SELECT M_Product_ID, Qty FROM M_RequisitionLine WHERE M_Requisition_ID=" + M_Requisition_ID;
		
		int year =   DB.getSQLValue(null, "select c_year_id from c_year where fiscalyear=(select substr(datedoc::text,0,5) from m_requisition where M_Requisition_ID="+ M_Requisition_ID+")");
		...
		try {
			...
			while (rs.next ())
			{
				DB.executeUpdate("Update C_Purchase_Plain_Line set Qty_Purch="+rs.getBigDecimal(2)+", Qty_Real = coalesce(Qty,0) - coalesce(Qty_Purch,0)  where M_Product_ID="+rs.getInt(1) + " AND C_Year_ID="+year ,null);
	...
	public int customizeValidActions(String docStatus, Object processing,
		...
		String resu;
		 if (docStatus.equals(DocumentEngine.STATUS_Completed)) {
			   resu = DB.getSQLValueString(null,"SELECT verifica_req("+getM_Requisition_ID()+") FROM M_Requisition WHERE M_Requisition_ID=" + getM_Requisition_ID());
	            if (resu.contains("Y")){
