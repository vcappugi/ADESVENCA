...
public class MMovement extends X_M_Movement implements DocAction
{
	...
	public String completeIt()
	{
		...
		//
		MProduct pro;
		MWarehouse warehouseO;
		MWarehouse warehouseD;
		MLocator locD;
		BigDecimal totalQty;
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			//Promedia todos los costos del almacen (Menos Nacionalizacion)
			...
			locD = new MLocator(Env.getCtx(),line.getM_LocatorTo_ID(), get_TrxName());
			warehouseD = new MWarehouse(Env.getCtx(), locD.getM_Warehouse_ID(), get_TrxName());
			
			//Totalizar Almacen (Destino) por Ronny Montaño
			if (!warehouseD.isNation())
			{
				M_LocatorTo_ID = line.getM_LocatorTo_ID();
				
				totalQty = MStorage.getQtyAvailable(0, line.getM_LocatorTo_ID(), line.getM_Product_ID(), 0, get_TrxName());

				if(totalQty != null){
					if(totalQty.signum()>0) {
						//Obtiene Costos Promedios de la Localización Destino
						getAvgCost(line.getM_LocatorTo_ID(), line.getM_Product_ID());
						
						//Elimina Registros de la localizacion Destino - producto
						deleteFromStorage(line.getM_LocatorTo_ID(), line.getM_Product_ID());
						
						MAttributeSetInstance instance = new MAttributeSetInstance(Env.getCtx(), 0, null); //Attribute Set Instance
						instance.save();
						
						MStorage storage = MStorage.getCreate(Env.getCtx(), line.getM_LocatorTo_ID(), line.getM_Product_ID(), instance.getM_AttributeSetInstance_ID(), null); //Creo Nuevo Registro Totalizado
					...
					&& product.isStocked() )
			{
				...
				if (line.getM_AttributeSetInstance_ID() == 0)
				{
					...
					for (int j = 0; j < mas.length; j++)
					{
						...
						else
						{
							...
							mst.setCost_USD(costoUsd.abs());
							mst.setCost_Pure(costoPure.abs());
							mst.setDateLastInventory(getDateReceived());
						...
						else
						{
							...
							MStorage mst = MStorage.get(getCtx(), line.getM_LocatorTo_ID(), line.getM_Product_ID(), M_AttributeSetInstanceTo_ID, get_TrxName());
							mst.setM_Product_Cost(costo.abs());
							mst.setCost_USD(costoUsd.abs());
							mst.setCost_Pure(costoPure.abs());
							mst.setDateLastInventory(getDateReceived());
						...
						trxFrom.setM_Product_Cost(costo);
						if (!trxFrom.save())
						{
				...
				{
					MLocator locator = new MLocator (getCtx(), line.getM_Locator_ID(), get_TrxName());
					if(!costo.equals(new BigDecimal(0))&&(!warehouseO.isNation()))
					   setAverageCost(line.getM_Locator_ID(), line.getM_Product_ID());;
					//Update Storage 
					...
					else
					{
						...
						MStorage mst = MStorage.get(getCtx(), line.getM_LocatorTo_ID(), line.getM_Product_ID(), line.getM_AttributeSetInstanceTo_ID(), get_TrxName());
						mst.setM_Product_Cost(costo.abs());
						mst.setCost_USD(costoUsd.abs());
						mst.setCost_Pure(costoPure.abs());
						mst.setDateLastInventory(getDateReceived());
			...
			//
			if(M_LocatorTo_ID > 0)
				deleteZeroStorage(line.getM_LocatorTo_ID(), line.getM_Product_ID());
