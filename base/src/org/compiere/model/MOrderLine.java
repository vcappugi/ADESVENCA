...
public class MOrderLine extends X_C_OrderLine
{
	...
	*/
	...
	public boolean setTax()
	{
		int prod = getM_Product_ID();
		int ii = DB.getSQLValue(null, "select * from adempiere.c_tax where isdefault='Y' and c_taxcategory_id = (select c_taxcategory_id from adempiere.m_product where m_product_id="+prod+")");
				...
				|| is_ValueChanged("M_Warehouse_ID")))
		{
			...
			if (product.isStocked())
			{
				...
				if (isInstance)
				{
					...
					if ((getQtyOrdered().compareTo(qty) > 0) && (!ord.getDeliveryRule().contains("F") ))
					{
