...
public class CalloutOrder extends CalloutEngine
{
	...
	public String bPartner (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		try
		{
			...
			if (rs.next())
			{
				...
				//Inactive from vcappugi for use common 
				...
				*/
				...
				/*
				 * Inactive for user common vcappugi
				 
				if (contID == 0)
					mTab.setValue("AD_User_ID", Env.getAD_User_ID(null));
				else
				{
				...
				*/
				mTab.setValue("AD_User_ID", Env.getAD_User_ID(ctx));
	...
	public String product (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		//
		if (steps) log.warning("fini");
		//Tax predefinido para el producto
		if (M_Product_ID != 0) {
			String sqlt = "select c_tax_id from c_tax where c_taxcategory_id = (select c_taxcategory_id from m_product where M_Product_ID="+M_Product_ID+") and isdefault='Y'";
			int c_tax_id = DB.getSQLValue(null, sqlt);
	...
	public String evaluate_budget(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value){
		//Evaluate Budget
		
		int cuenta = DB.getSQLValue(null, "Select C_ElementValue_ID from M_Product_Budg where M_PRoduct_ID=" + mTab.getValue("M_Product_ID"));
		String sql = "select COALESCE(realamount,0) from C_BudgetPublic_Line Where C_budget_Account_ID="+cuenta+" AND C_BudgetPublic_ID ="+ mTab.getValue("C_BudgetPublic_ID");
		BigDecimal disponible = DB.getSQLValueBD(null, sql);
		
		if (disponible==null)
			disponible = new BigDecimal(0);
		if (disponible.compareTo((BigDecimal) mTab.getValue("LineNetAmt")) ==-1) 
			mTab.setValue("IsBudgetCover","N"); 
		 
		else
			mTab.setValue("IsBudgetCover","Y");
		int cuantos = DB.getSQLValue(null, "select count(c_orderline_id) from c_orderline where c_order_id ="+mTab.getValue("C_Order_ID")+" and isbudgetcover='N'");
		if (!(cuantos >0)&& (disponible.compareTo((BigDecimal) mTab.getValue("LineNetAmt")) !=-1) )
			DB.executeUpdate("UPDATE C_Order set IsBudgetCover='Y' where C_Order_ID=" + mTab.getValue("C_Order_ID"));
		else
			DB.executeUpdate("UPDATE C_Order set IsBudgetCover='N' where C_Order_ID=" + mTab.getValue("C_Order_ID"));
		return "";
	}
	...
	public String tax (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		int C_Tax_ID=0;
				if (M_Product_ID != 0) {
					String sqlt = "select c_tax_id from c_tax where c_taxcategory_id = (select c_taxcategory_id from m_product where M_Product_ID="+M_Product_ID+") and isdefault='Y'";
					C_Tax_ID = DB.getSQLValue(null, sqlt);
					mTab.setValue("C_Tax_ID", C_Tax_ID);
				}
		//
	...
	public String amt (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		//PriceList = (BigDecimal)mTab.getValue("PriceEntered");
