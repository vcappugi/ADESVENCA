...
import org.pentanet.model.MMailData;
import org.pentanet.model.X_AD_Sequence_Org;
...
public class MInvoice extends X_C_Invoice implements DocAction
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if(getDocumentNo()==null){
			newDoc = true;
		}else if(getDocumentNo().length()>1){
			if(getDocumentNo().substring(0,1).equals("<"))
				newDoc = true;
		}
		
		if(newDoc == true)
		{
			String sql = "SELECT AD_Sequence_Org_ID FROM AD_Sequence_Org WHERE IsActive='Y' AND C_DocType_ID=? AND AD_Org_ID=? AND IsDocumentNo='Y'";
			int  AD_Sequence_Org_ID = DB.getSQLValue(null, sql, getC_DocTypeTarget_ID(),getAD_Org_ID());
			
			if(AD_Sequence_Org_ID>0)
			{
				X_AD_Sequence_Org seq = new X_AD_Sequence_Org(getCtx(), AD_Sequence_Org_ID, get_TrxName());
				setDocumentNo(seq.getCurrentNext()+"");
	...
	protected boolean beforeDelete ()
	{
		...
		String valida =  DB.getSQLValueString(null,"SELECT DocStatus FROM C_Invoice WHERE C_Invoice_ID=" + getC_Invoice_ID());
	...
	{
		// Agregado para notificar cuando complete relaciones de gastos WL2015

		
		String send = DB.getSQLValueString(null, "SELECT send FROM Document_Info("+get_Table_ID()+","+getC_Invoice_ID()+")");
		
		if(send != null){
			if( send.contains("Y"))
			{
				//Notificar via Correo al Empleado
				int R_MailText_ID = DB.getSQLValue(null, "SELECT R_MailText_ID FROM R_MailText WHERE value = 'RL_Complet'");
				
				MMailText plantilla = new MMailText(Env.getCtx(), R_MailText_ID, null);
			    MMailData datos = plantilla.getMailDataVar(getC_Invoice_ID(), null);
		...
		//Agregado para verificar si la factura ya tiene comprobante de retención
		String sql ="SELECT COUNT(*) FROM LCO_Comprobant WHERE C_Invoice_ID="+getC_Invoice_ID();
		int comp = DB.getSQLValue(null, sql);
		
		if((getWithholdingAmt().signum()>0) && (comp==0) && isSOTrx()==false && !getDocumentNo().contains("R")) //Retencion > 0, Comprobantes = 0, Facturas de Compras
		{
			m_processMsg = "No se ha generado el Comprobante de Retención";
			return DocAction.STATUS_Invalid;
		}
		//////
		
		//Validate HES / Num_Control
		MDocType dt = new MDocType(Env.getCtx(), getC_DocType_ID(), null);
		if((dt.getDocBaseType().equals(X_C_DocType.DOCBASETYPE_ARInvoice)) 
				&& (!getDocumentNo().contains("R"))
				&& (!validate())){
		...
		//Users
		if(!isSOTrx()){
			sql = "SELECT AD_User_Rev_ID, DateRev FROM User_Aprob_RG(?,?)";
			...
			try
			{
				...
				while (rs.next())
				{
					if(rs.getInt(1)!=0){
						...
						setDateRev(rs.getTimestamp(2));
					}else{
						setAD_User_Rev_ID(Env.getAD_User_ID(getCtx()));
						setDateRev(Env.getContextAsDate(getCtx(), "#Date"));
					}
					setAD_User_Aprob_ID(Env.getAD_User_ID(getCtx()));
					setDateAprob(Env.getContextAsDate(getCtx(), "#Date"));
				}
				rs.close();
				pstmt.close();
			}
	...
	private boolean validate() {
		...
		if(getSheet_In()!=null){
			...
			sql = "SELECT COUNT(*) FROM C_Invoice WHERE Sheet_In = ? AND NOT C_Invoice_ID = ? AND IsSOTrx='Y'";
			
			if(DB.getSQLValue(null, sql, new Object[]{getSheet_In(),getC_Invoice_ID()})>0){
				m_processMsg = "Número de HES Duplicado";
				return false;
			}
		}
		
		//Num_Control
		if(getnum_control()!=null){
			
			sql = "SELECT COUNT(*) FROM C_Invoice WHERE Num_Control = ? AND NOT C_Invoice_ID = ? AND IsSOTrx='Y'";
			
			if(DB.getSQLValue(null, sql, new Object[]{getnum_control(),getC_Invoice_ID()})>0){
				m_processMsg = "Número de Control Duplicado";
				return false;
			}
		}
...
//		MOrgInfo counterOrgInfo = MOrgInfo.get(getCtx(), counterAD_Org_ID);
	...
	public boolean voidIt()
	{
		...
		if (DOCSTATUS_Drafted.equals(getDocStatus())
			...
			|| isSOTrx()) //Agregado para Anular Facturas de Venta
		{
			if(isSOTrx()) //Agregado para Anular Facturas de Venta
			{
				setGrandTotal(Env.ZERO);
				setTotalLines(Env.ZERO);
				setTotalTax(Env.ZERO);
				setBaseTax(Env.ZERO);
				setTotal_Invoice(Env.ZERO);
				setSheet_In("");
				save();
				DB.executeUpdate("DELETE FROM Fact_Acct WHERE AD_Table_ID = " + get_Table_ID() + " AND Record_ID = " + getC_Invoice_ID(), get_TrxName());
			}
	...
	public boolean reverseCorrectIt()
	{
		...
		//Deberia aqui modificar para que no genere nuevo nro de documento ??
		// y colocar la fecha real de la factura reversada ???
		//Podria colocar el mismo nro de documento original añadiendole una R
		reversal.setDocumentNo(getDocumentNo() + "R");
		//Pero siempre actualiza el nro consecutivo de la factura por lo que 
		//Debo restarlo ???
		...
		reversal.setDateAcct( Env.getContextAsDate(getCtx(), ""));   //Fecha actual y no la fecha original de la factura
		reversal.setDateInvoiced(Env.getContextAsDate(getCtx(), ""));
