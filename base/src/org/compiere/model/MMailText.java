...
import org.pentanet.model.MRMailTextVar;
import org.pentanet.model.X_R_MailTextVar;
...
public class MMailText extends X_R_MailText
{
	...
	private org.adempiere.util.EncryptRLMH.Encrypt Encrypt = null;
	 ...
	 *	@return body
	 */
	public MMailData[] getMailData()
	{		
		List<MMailData> listVariable = new ArrayList<MMailData>();
		String sqlVar = getSQLVar();
		...
		{			
			...
			if (rs.wasNull() == false){ //Verifica si el Resulset esta null

				while (rs.next())
				{
					String body = getMailText();
					String tittle = getMailHeader();
					String email = null;
					colum = rs.getMetaData().getColumnCount(); // Obtiene la Cantidad de columnas
					
					for(int i = 1; i <= colum; i++)
					{
						String regex = "@_" + rs.getMetaData().getColumnName(i).toUpperCase() + "@";
						String replacement = rs.getObject(i) + "";
						body = body.replace(regex, replacement);
						tittle = tittle.replace(regex, replacement);
						
						if(rs.getMetaData().getColumnName(i).equals("email"))
							email = rs.getObject(i) + "";
					}
					
					listVariable.add(new MMailData(email, tittle, body, null));
		...
		return listVariable.toArray(new MMailData[listVariable.size()]);
	}
	 ...
	 *	@return void
	 */
	public MMailData getMailDataVar(int Record_ID, String Aprobado)
	{	
		return getMailDataVar(""+Record_ID, Aprobado);
	}
	...
	public MMailData getMailDataVar(String Record, String Aprobado)
	{	
		String whereClause = " R_MailText_ID = ? ";
		List<MRMailTextVar> list = new Query(getCtx(), MRMailTextVar.Table_Name, whereClause, get_TrxName())
		.setParameters(new Object[]{getR_MailText_ID()})
		.list();
		
	    String body = getMailText();
		String tittle = getMailHeader();
		String email = null;
		ArrayList<InternetAddress> m_cc = null;
		InternetAddress ia = null;
		
		for(MRMailTextVar var : list)
		{
			log.warning("nombre de la variable: " + var.getName());
			
			//Sustituir Record_ID del SQLVar de la Variable
			String sql = null;
			String valor = null;
			Encrypt = new org.adempiere.util.EncryptRLMH.Encrypt();
			
			if (var.getSQLVar().equals("@_APPROVAL_CODE@")){
				valor = MWFActivity.approval_Code + "&" + (var.getName().toString().equals("Y") ? 1 : 0);
			}else{
				if (var.getSQLVar().equals("@_LastApro@")){
					valor = DB.getSQLValueString(get_TrxName(), "select usuario from description_employee("+MWFActivity.getUpBy()+",2)");
				}else{
				sql = var.getSQLVar().replace("@_RECORD_ID@", Record);
				valor = DB.getSQLValueString(get_TrxName(), sql);
				}
			}
			if(var.isEncrypted()){
				valor = Encrypt.getEncrypt(valor);
			}
			
			if(var.isMail()) //Correo
			{
				email = valor;
			}
			else if (var.isMailCc())
			{
				if(m_cc == null)
				...
				if(valor!=null){
					try {
						ia = new InternetAddress (valor, true);
			...
			else
			{
				...
				String regex = "@_" + var.getName().toString() + "@";
				String replacement = valor.toString();
				body = body.replace(regex, replacement);
				tittle = tittle.replace(regex, replacement);
			}
		...
		//Sustituir Variables Aprobado en la Plantilla
		if(Aprobado!=null)
			body = body.replace("@_Aprobado@", Aprobado);
				
		MMailData md = new MMailData(email, tittle, body, m_cc);
