	...
	private void setUser_ID (Integer User_ID)
	{
		...
		if (AD_User_ID == 0 && resp.isInvoker())
		{
			...
			if (m_po != null && m_po instanceof DocAction)
			{
				...
				int mwfp = DB.getSQLValue(null,"SELECT COALESCE((select Count(*) from AD_WF_Process where "
						+ "AD_Table_ID = "+ da.get_Table_ID() +" AND Record_ID = "+da.get_ID()+"),0)");
				if(mwfp == 0){
					AD_User_ID = Env.getAD_User_ID(getCtx());
				}else{
					AD_User_ID = DB.getSQLValue(null,"SELECT AD_User_ID from AD_WF_Process WHERE "
							+ "AD_Table_ID = "+ da.get_Table_ID() +" AND Record_ID = "+da.get_ID());
				}
