...
public class MWFActivity extends X_AD_WF_Activity implements Runnable
{
	 ...
	 * */
	public static String approval_Code = null;
	 ...
	 */
	private boolean notifyUser(MWFResponsible resp) {
		user = getUpdatedBy();
		if(resp.getAD_Role_ID()==0)
			return false;
		
		MRole rol = new MRole(Env.getCtx(), resp.getAD_Role_ID(), get_TrxName());
		if(!rol.isEmail())
			return false;
		
		MUserRoles[] userRoles = MUserRoles.getOfRole(Env.getCtx(), resp.getAD_Role_ID());
		
		for(MUserRoles ur : userRoles) { //for
			
			if(ur.isActive()){ //User Active
				MUser user = new MUser(Env.getCtx(), ur.getAD_User_ID(), get_TrxName());
				
				approval_Code = ur.getAD_User_ID() + "&" + user.getName() + 
						"&" + ur.getAD_Role_ID() + "&" + getRecord_ID() + "&" + getAD_Table_ID();
				
				if(user.getEMail()!=null && user.isEmail()) { //User Email
					// indicador para enviar el correo segun el tipo de documento
					// 1 --> Enviar Email (tiene el tipo de documento o no tiene asignado)
					// 0 --> No tiene el tipo de documento asignado 
					
					String sql = "SELECT * FROM COALESCE((SELECT (CASE WHEN (SELECT Count(C_DocType_ID) FROM AD_User_DocType ud "
							+ "WHERE AD_User_ID = "+ ur.getAD_User_ID() +") > 0 THEN  "
									+ "CASE WHEN (SELECT (CASE WHEN C_DocType_ID > 0 THEN 1 END) FROM AD_User_DocType ud "
									+ "INNER JOIN document_info("+ getAD_Table_ID() +", "+ getRecord_ID() +") di ON di.doctype_id = ud.C_DocType_ID "
									+ "WHERE AD_User_ID = "+ ur.getAD_User_ID() +") = 1 THEN 1 ELSE 0 END "
									+ "END)), 1)";
					
					int ind = DB.getSQLValue(null, sql);
					
					// verifica si tiene asignado tipo de documentos
					if(ind == 1){
						int R_MailText_ID = DB.getSQLValue(null, "SELECT R_MailText_ID FROM R_MailText WHERE value = 'WF_Aprob'");
						
						MMailText plantilla = new MMailText(Env.getCtx(), R_MailText_ID, null);
					    MMailData datos = plantilla.getMailDataVar(getAD_Table_ID() + "::numeric," + getRecord_ID() + "::numeric", null);
					    ...
					    correo.sendEmail(user.getEMail(), plantilla.getMailHeader(), datos.getBody());
					}// ind
