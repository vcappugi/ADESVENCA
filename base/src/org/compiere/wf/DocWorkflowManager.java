...
public class DocWorkflowManager implements DocWorkflowMgr
{
	...
	*/
	 ...
	 * 
	 */
	public static boolean processStartWF(PO document) {
		
		int AD_Process_ID = (new MColumn(Env.getCtx(), MColumn.getColumn_ID(document.get_TableName(),"DocAction"), document.get_TrxName())).getAD_Process_ID();
		if(AD_Process_ID==0)
			return false;
		...
		MWorkflow wf = new MWorkflow (Env.getCtx(), process.getAD_Workflow_ID(), document.get_TrxName());
		
		ProcessInfo pi = new ProcessInfo (wf.getName(), process.getAD_Process_ID(), 
				document.get_Table_ID(), document.get_ID());
		pi.setAD_User_ID (Env.getAD_User_ID(Env.getCtx()));
		pi.setAD_Client_ID(document.getAD_Client_ID());
		
		wf.start(pi, document.get_TrxName());
