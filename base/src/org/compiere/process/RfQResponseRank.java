...
public class RfQResponseRank extends SvrProcess
{
	...
	protected String doIt () throws Exception
	{
		...
		if (responses.length == 0)
			throw new IllegalArgumentException("No hay cotizaciones completas para el proceso");
		if (responses.length < 3 )
		{
			responses[0].setIsSelectedWinner(true);
			responses[0].saveEx();
			//return "Only one completed RfQ Response found";
			ADialog.error(Env.WINDOW_MAIN, null, "No existen Suficientes Respuestas de Cotizaciones");
			return "No hay suficientes respuestas de cotizaciones";
		}
	...
	private void rankLines (MRfQ rfq, MRfQResponse[] responses)
	{
		...
		if (winner != null)
		{
			...
			rfq.setC_BPartner_ID(winner.getC_BPartner_ID());
			rfq.save();
			MBPartner bp = new MBPartner(getCtx(),winner.getC_BPartner_ID(),get_TrxName());
			ADialog.info(Env.WINDOW_MAIN, null,"El Ganador del proceso es el proveedor: " + bp.getName() );
		}
	}	//	rankLines
		...
		for (int ii = 0; ii < responses.length; ii++)
		{
				...
				&& response.getPrice().compareTo(Env.ZERO) > 0)
			{
				if (response.isSelectedWinner() != (ranking == 1)) {
					...
					//Modify Vcappugi
				    rfq.setC_BPartner_ID(response.getC_BPartner_ID());
				    rfq.save();}
					//End Modify vcappugi
				response.setRanking(ranking);
				//
