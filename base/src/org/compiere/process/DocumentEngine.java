...
//		Locked
		...
		int AD_Client_ID, int AD_Table_ID, int Record_ID, boolean force, String trxName)
	{
		...
		if(Ini.isPropertyBool(Ini.P_SERVEROFF)) // No requiere el Sevidor
			{
			try
				{
				Properties p = Env.getCtx();
				postImmediate_serverOff(p,AD_Client_ID,AD_Table_ID,Record_ID,force,null);		
				
//				Server server = (Server)Proxy.newProxyInstance(Server.class.getClassLoader(),
//						new Class[]{Client.class}, new EmbeddedServerProxy());
...
//								AD_Table_ID, Record_ID, force, null); // don't pass transaction to server
	...
	public static String postImmediate_Original (Properties ctx, 
			int AD_Client_ID, int AD_Table_ID, int Record_ID, boolean force, String trxName)
		{
			String error = null;
			if (MClient.isClientAccounting()) {
	 ...
	 *  @return null, if success or error message
	 */
	public static String postImmediate_serverOff (Properties ctx, 
		int AD_Client_ID, int AD_Table_ID, int Record_ID, boolean force, String trxName)
	{
		MAcctSchema[] ass = MAcctSchema.getClientAcctSchema(ctx, AD_Client_ID);
