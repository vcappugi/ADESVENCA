...
import org.compiere.model.MStorage;   //Adicion para costo de almacen
...
public class Doc_InOut extends Doc
{
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		if (getDocumentType().equals(DOCTYPE_MatShipment) && isSOTrx())
		{
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				//vcappugi
				/*
				 * Busca Costo del Almacen, si no lo encuentra, busca costo predefinido del producto	
				 */
						String sql1 = "SELECT M_Product_Cost from M_Storage WHERE M_Locator_ID ="+line.getM_Locator_ID()+ " AND M_Product_ID=" + line.getM_Product_ID() + " AND M_Product_Cost <> 0";
						BigDecimal costs = DB.getSQLValueBD(getTrxName(), sql1);
		...
		else if (getDocumentType().equals(DOCTYPE_MatReceipt) && !isSOTrx())
		{
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				{*/
					...
					MInOutLine inoutline = new MInOutLine (getCtx(),line.get_ID() , getTrxName());
					MInvoiceLine fline = MInvoiceLine.getOfInOutLine(inoutline);
					
					int C_OrderLine_ID = line.getC_OrderLine_ID();
					
					if (fline != null){
						costs = fline.getLineNetAmt();
						
					}
					else // Low - check if c_orderline_id is valid
					{
					    ...
					    /*
						 * Modificacion para llenar costo de mercancia en el storage
						
					    String act_c ="UPDATE M_STORAGE SET M_Product_Cost= ? WHERE M_LOCATOR_ID = ? AND M_PRODUCT_ID=? AND QTYonHand=?";
					    PreparedStatement pstmtt = DB.prepareStatement(act_c, getTrxName());
					    try {
							pstmtt.setBigDecimal(1, costs);  //costo del producto
							pstmtt.setInt(2, line.getM_Locator_ID());  //ubicacion
							pstmtt.setInt(3, line.getM_Product_ID());  //Producto
							pstmtt.setBigDecimal(4, line.getQty());   //Cantidad de producto
	 ...
	 *  
	 */
	public void createFactsNew (MAcctSchema as)
	{
		...
		//Si no hay ordenes atadas
		if (getC_Currency_ID() > 0)
			moneda = getC_Currency_ID();
		else
			moneda = as.getC_Currency_ID();
		 

		MInOut inout = new MInOut(getCtx(),get_ID(),getTrxName());
		
		int sdn = inout.getC_BPartner_ID();
		int Record = inout.get_ID();
		String fecha_acct = DB.getSQLValueString(null,"select to_char(DateAcct, 'dd/mm/yyyy') from M_InOut where M_InOut_id =" + Record);
		int period_id = DB.getSQLValue(null, "select c_period_id from c_period where startdate <= '"+fecha_acct+"'  and enddate >= '"+fecha_acct+"'");

		//sql
		String sql = "SELECT ML.M_PRODUCT_ID, (currencyconvert ((abs(ML.MOVEMENTQTY) * (OL.PRICEENTERED)), O.C_Currency_ID, "+as.getC_Currency_ID()+", O.DateAcct, null, 1000000, 1000000)) AS LINENETAMT, "
				+ "M.DATEACCT, (select vc.account_id from c_validcombination vc where vc.c_validcombination_id =AC.P_Order_Invt_Acct) as invtrans,  "
				+ "(select vc.account_id from c_validcombination vc where vc.c_validcombination_id =AC.P_Order_InvtPC_Acct) as invtranscontra, "
				+ "M.DESCRIPTION,  M.C_ACTIVITY_ID, (abs(ML.MOVEMENTQTY) * (OL.PRICEENTERED))  FROM M_INOUTLINE ML "
				+ "INNER JOIN C_ORDERLINE OL ON OL.C_ORDERLINE_ID = ML.C_ORDERLINE_ID "
				+ "INNER JOIN C_ORDER O ON O.C_ORDER_ID = OL.C_ORDER_ID "
				+ "INNER JOIN M_PRODUCT_ACCT AC ON AC.M_PRODUCT_ID = ML.M_PRODUCT_ID "
				+ "INNER JOIN M_INOUT M ON M.M_INOUT_ID = ML.M_INOUT_ID WHERE ML.M_INOUT_ID=" + Record;
		...
		try
		{
			...
			cont1.setC_Currency_ID(moneda);
			cont1.setAD_Table_ID(tabla);
			cont1.setC_BPartner_ID(sdn);
			cont1.setAccount_ID(rs.getInt(5));
			if (inout.getDocStatus().equals("CO") || inout.getDescription().contains("<-")) {
			      cont1.setAmtAcctDr(rs.getBigDecimal(2));
			      cont1.setAmtSourceDr(rs.getBigDecimal(8));
			} else if (inout.getDocStatus().equals("RE")) {
			      cont1.setAmtAcctCr(rs.getBigDecimal(2));
			      cont1.setAmtSourceCr(rs.getBigDecimal(8));
			}
			cont1.setC_AcctSchema_ID(esquema); 
			cont1.setDateAcct(rs.getTimestamp(3));
			cont1.setDateTrx(rs.getTimestamp(3));
			cont1.setDescription(rs.getString(6));
			cont1.setC_Activity_ID(rs.getInt(7));
			cont1.setRecord_ID(Record);   
			cont1.setC_Period_ID(period_id);
			cont1.setPostingType("A");
			cont1.setM_Product_ID(rs.getInt(1));
			cont1.save();
	
			//HABER
			MFactAcct cont = new MFactAcct(getCtx(), 0, getTrxName());  //creo un registro en fact_acct
			cont.setC_Currency_ID(moneda);
			cont.setAD_Table_ID(tabla);
			cont.setC_BPartner_ID(sdn);
			cont.setAccount_ID(rs.getInt(4));
			if (inout.getDocStatus().equals("CO") || inout.getDescription().contains("<-")) {
			      cont.setAmtAcctCr(rs.getBigDecimal(2));
			      cont.setAmtSourceCr(rs.getBigDecimal(8));
			} else if (inout.getDocStatus().equals("RE")) {
			      cont.setAmtAcctDr(rs.getBigDecimal(2));
			      cont.setAmtSourceDr(rs.getBigDecimal(8));
			}
			cont.setC_AcctSchema_ID(esquema); 
			cont.setDateAcct(rs.getTimestamp(3));
			cont.setDateTrx(rs.getTimestamp(3));
			cont.setDescription(rs.getString(6));
			cont.setC_Activity_ID(rs.getInt(7));
			cont.setRecord_ID(Record);   
			cont.setC_Period_ID(period_id);
			cont.setPostingType("A");
			cont.setM_Product_ID(rs.getInt(1));
			cont.save();
