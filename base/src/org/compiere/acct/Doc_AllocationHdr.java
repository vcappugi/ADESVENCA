...
public class Doc_AllocationHdr extends Doc
{
	...
	private DocLine[] loadLines(MAllocationHdr alloc)
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			String IsPayDirect = DB.getSQLValueString(getTrxName(), "SELECT ISPAYDIRECT FROM C_INVOICE WHERE C_INVOICE_ID= "+line.getC_Invoice_ID());
			
			if(line.getC_Invoice_ID() != 0){ //Para actualizar CC y Poyecto en las Lineas desde Factura
				MInvoice inv = new MInvoice(getCtx(), line.getC_Invoice_ID(), getTrxName());
				line.setC_Activity_ID(inv.getC_Activity_ID());
				line.setC_Project_ID(inv.getC_Project_ID());
				line.save();
			}
			
			if ((IsPayDirect == null) || (IsPayDirect.equalsIgnoreCase("N"))){
				...
				if (line.getC_Payment_ID() != 0)
				{
					...
					if(payment.getC_Activity_ID()!=0){ //Para actualizar CC en las Lineas por el del pago
						line.setC_Activity_ID(payment.getC_Activity_ID()); 
						line.save();
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			if (line.getC_Payment_ID() != 0) {
				...
				activity = payment.getC_Activity_ID();  //Tomo CC del pago
			}
			...
			else if (invoice.isSOTrx())
			{
				...
				// then don't post
				//obtiene si el C_tax esta marcado para usar el esquem contable
				sql ="SELECT COALESCE((select isCurrencyscheme from LCO_InvoiceWithholding l, C_Tax t where l.C_Tax_ID= t.C_Tax_ID "
						+ "  AND isCurrencyscheme = 'Y' AND l.C_AllocationLine_ID = "+ line.get_ID()+"),'N')";
				lco_w = DB.getSQLValueString(getTrxName(), sql).equals("Y") ? true : false;
				...
				if ((!as.isPostIfClearingEqual()) && acct_unallocated_cash != null && acct_unallocated_cash.equals(acct_receivable) && (!isInterOrg)) {
					...
					allocationSource = (line.getWriteOffAmt());  
					...
					if (line.getC_Payment_ID() != 0)
					{
						...
						if(lco_w && fl != null){
							amtdr =amtdr.add(fl.getAmtAcctDr());
							fl.setAmtSourceDr(fl.getAmtAcctDr());
							fl.setC_Currency_ID(as.getC_Currency_ID());
						}
				...
				{
					//Suma los Impuesto detallados por documento
					@SuppressWarnings("unused")
					BigDecimal	WriteOffAmt = new BigDecimal(0);
					WriteOffAmt= DetailWriteOffAmt(as, line.get_ID(), line.getC_Invoice_ID(),new BigDecimal(0), new BigDecimal(0));
					
					fl = fact.createLine (line, getAccount(Doc.ACCTTYPE_WriteOff, as),
						getC_Currency_ID(), line.getWriteOffAmt(), null);
					
					fl.setAmtAcctDr(WriteOffAmt);
				...
				if (as.isAccrual())
				{
					...
					if (payment != null){
						...
						// Diferencias de Monedas entre el pago y la factura ***rmontano
						if (payment.getC_Currency_ID() != invoice.getC_Currency_ID()){
							
							BigDecimal amount = MConversionRate.convert(getCtx(), allocationSource, payment.getC_Currency_ID(), 
									invoice.getC_Currency_ID(),getAD_Client_ID(), getAD_Org_ID());
							
							fl = fact.createLine (line, bpAcct,
									invoice.getC_Currency_ID(), null, amount);	//	payment currency
							
							fact.setIsMultiCurrency(true);
					...
					if(lco_w && fl != null){
						amtdr = amtdr.subtract(fl.getAmtAcctCr());
						if (amtdr.signum() != 0){
							fl.setAmtSourceCr(fl.getAmtAcctCr().add(amtdr));
							fl.setAmtAcctCr(fl.getAmtAcctCr().add(amtdr));
						}else{
							fl.setAmtSourceCr(fl.getAmtAcctCr());
						}
						fl.setC_Currency_ID(as.getC_Currency_ID());
					}
					...
					if (flForRGL != null){
						...
						flForRGL.setC_Activity_ID(activity);
					}
				}
				else	//	Cash Based
				{
			...
			else
			{
				...
				if (as.isAccrual())
				{
					MBPartner bp = new MBPartner(Env.getCtx(), getC_BPartner_ID(), null);
					
					//if(bp.isEmployee()){
					//	bpAcct = getAccount(Doc.ACCTTYPE_V_EmployeeCxC,as); 
					//}else{
					...
					if (payment != null){
						...
						if (payment.getC_Currency_ID() != invoice.getC_Currency_ID()){
							...
							fl = fact.createLine (line, bpAcct,
									invoice.getC_Currency_ID(), amount, null);		//	payment currency
		...
		if ( (!as.isPostIfClearingEqual()) && p_lines.length > 0 && (!isInterOrg)) {
			...
			for (FactLine factLine : factlines) {
				...
				netBalance = netBalance.add(factLine.getAmtAcctDr()).subtract(factLine.getAmtAcctCr());
	...
	private BigDecimal DetailWriteOffAmt(MAcctSchema as, int line, int inv_id, BigDecimal amtbase , BigDecimal amtimp ){
		
		BigDecimal retvalue = new BigDecimal(0);
		//BigDecimal tottax = new BigDecimal(0);
		//Fact fact = new Fact(this, as, Fact.POST_Actual);
		MInvoice invoice = null;
		invoice = new MInvoice (getCtx(), inv_id, getTrxName());
		
		int currency = invoice.getC_Currency_ID();
		...
		try
		{
			...
			while (rs.next()) {
				...
				retvalue = retvalue.add(MConversionRate.convert(getCtx(),amount , currency , as.getC_Currency_ID(), 
		...
		/*BigDecimal conv_imp = MConversionRate.convert(getCtx(), amtimp , currency , as.getC_Currency_ID(), 
				invoice.getAD_Client_ID(), invoice.getAD_Org_ID());
		tottax = tottax.add(amtbase);
		retvalue = amtbase.add(conv_imp.subtract(tottax));*/
		...
		MInvoice invoice, BigDecimal allocationSource, BigDecimal allocationAccounted)
	{
		...
		try
		{
			...
			pstmt = DB.prepareStatement(sql, getTrxName());
			pstmt.setInt(1, invoice.getC_Invoice_ID());
			pstmt.setInt(2, invoice.getC_Currency_ID());
