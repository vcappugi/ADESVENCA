...
public class Doc_Movement extends Doc
{
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			String isRes = DB.getSQLValueString(getTrxName(), "SELECT IsResult FROM M_MovementLine where M_MovementLine_ID="+ line.get_ID());

			if (isRes.contentEquals("N") ) {
				...
				{
					if(j == 1){
						costs = DB.getSQLValueBD(getTrxName(), "SELECT Cost_Pure FROM M_MOVEMENTLINE WHERE M_MOVEMENTLINE_ID=" + line.get_ID());
						setC_Currency_ID (as.getC_Currency_ID());
					}else{
						costs = DB.getSQLValueBD(getTrxName(), "SELECT Cost_USD FROM M_MOVEMENTLINE WHERE M_MOVEMENTLINE_ID=" + line.get_ID());
						setC_Currency_ID(100);
					}
						//line.getProductCosts(as, line.getAD_Org_ID(), true, "M_MovementLine_ID=? AND IsSOTrx='N'");
					//Costo de Contabilizacion de almacenaje
					//Por vcappugi
					//
					//costo = DB.getSQLValueBD(getTrxName(),"SELECT M_Product_cost FROM M_STORAGE WHERE M_PRODUCT_ID ="+line.getM_Product_ID()+" AND DATELASTINVENTORY='"+line.getDateDoc() +"' AND M_LOCATOR_ID="+line.getM_Locator_ID()); 
					
					//end vcappugi
					// end MZ
					if (costs.compareTo(new BigDecimal(0)) > 0)
						if ( line.getQty().compareTo(new BigDecimal(0)) > 0 )
						      costs = costs.multiply(line.getQty());
						else
							costs = (costs.multiply(line.getQty())).abs().negate();
					// end MZ
					costs = costs.setScale(2, BigDecimal.ROUND_HALF_UP);
