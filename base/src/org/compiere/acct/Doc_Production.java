...
public class Doc_Production extends Doc
{
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			X_M_ProductionLine pl = new X_M_ProductionLine(getCtx(), line.get_ID(), getTrxName());
			
			//	Calculate Costs
			BigDecimal costs = null;
			BigDecimal costs_USD = null;
			BigDecimal costs_Pure = null;
			...
			{	
				...
				if (line.isProductionBOM())
				{
					...
					BigDecimal bomCost_USD = Env.ZERO;
					for (int ii = 0; ii < p_lines.length; ii++)
					{
						...
						if (!line0.isProductionBOM()){
							...
							X_M_ProductionLine pl0 = new X_M_ProductionLine(getCtx(), line0.get_ID(), getTrxName());
							bomCost = bomCost.add(pl0.getCost_Pure().setScale(2, BigDecimal.ROUND_HALF_UP).multiply(line0.getQty()));
							bomCost_USD = bomCost_USD.add(pl0.getCost_USD().setScale(2, BigDecimal.ROUND_HALF_UP).multiply(line0.getQty()));
						}
					}
					costs_Pure = bomCost.negate();
					costs_USD = bomCost_USD.negate();
				    ...
				    if (line.getQty().compareTo(mpp.getProductionQty())!=0) {
				    	// if the line doesn't correspond with the whole qty produced then apply prorate
				    	costs_Pure = costs_Pure.multiply(line.getQty()).divide(mpp.getProductionQty(), 2, BigDecimal.ROUND_HALF_UP);
				    	costs_USD = costs_USD.multiply(line.getQty()).divide(mpp.getProductionQty(), 2, BigDecimal.ROUND_HALF_UP);
				    }
				}
				else{
					//costs = line.getProductCosts(as, line.getAD_Org_ID(), false);
					costs_Pure = pl.getCost_Pure().setScale(2, BigDecimal.ROUND_HALF_UP).multiply(line.getQty());
					costs_USD = pl.getCost_USD().setScale(2, BigDecimal.ROUND_HALF_UP).multiply(line.getQty());
				}
			...
			for (int j = 1; j <= 2; j++)
			{
				...
				if(costs.compareTo(new BigDecimal(0)) !=0){
					
					//  Inventory       DR      CR
					fl = fact.createLine(line,
						line.getAccount(ProductCost.ACCTTYPE_P_Asset, as),
						getC_Currency_ID(), costs);
					if (fl == null)
					{
