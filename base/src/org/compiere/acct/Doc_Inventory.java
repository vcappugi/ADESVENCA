...
public class Doc_Inventory extends Doc
{
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			String isRes = DB.getSQLValueString(getTrxName(), "SELECT IsResult FROM M_InventoryLine where M_InventoryLine_ID="+ line.get_ID());
			...
			if (isRes.contentEquals("N") ) {
				...
				{
					if(j == 1){
						costs = DB.getSQLValueBD(getTrxName(), "SELECT Cost_Pure FROM M_INVENTORYLINE  WHERE M_INVENTORYLINE_ID=" + line.get_ID());
						setC_Currency_ID (as.getC_Currency_ID());
					}else{
						costs = DB.getSQLValueBD(getTrxName(), "SELECT Cost_USD FROM M_INVENTORYLINE  WHERE M_INVENTORYLINE_ID=" + line.get_ID());
		...
		*/
				...
				}//para recorrer los costos puros en bolivares y dolares
			}
		}
		//
