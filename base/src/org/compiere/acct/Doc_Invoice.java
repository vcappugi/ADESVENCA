...
public class Doc_Invoice extends Doc
{
	...
	private BigDecimal taxSchema;
	private BigDecimal taxtotalSchema;
	...
	private DocTax[] loadTaxes()
	{
		...
		String sql = "SELECT it.C_Tax_ID, t.Name, t.Rate, it.TaxBaseAmt, it.TaxAmt, t.IsSalesTax, t.IsCurrencyScheme, i.C_Activity_ID  "
				+ " FROM C_InvoiceTax it INNER JOIN C_Tax t ON t.C_Tax_ID = it.C_Tax_ID "
				+ " INNER JOIN C_Invoice i ON i.C_Invoice_ID = it.C_Invoice_ID WHERE t.IsActivity='N' AND "
				+ " it.C_Invoice_ID=? UNION "
				+ " SELECT il.C_Tax_ID, t.Name, t.Rate, sum(il.LineNetAmt) as TaxBaseAmt, sum(il.TaxAmt) as TaxAmt, t.IsSalesTax, t.IsCurrencyScheme,"
				+ " (CASE WHEN il.c_activity_id is null THEN i.c_activity_id ELSE il.c_activity_id END) as C_Activity_ID "
				+ " FROM C_InvoiceLine il INNER JOIN C_Invoice i ON i.C_Invoice_ID = il.C_Invoice_ID "
				+ " INNER JOIN C_Tax t ON t.C_Tax_ID = il.C_Tax_ID WHERE t.IsActivity='Y' AND il.C_Invoice_ID=? "
				+ " GROUP BY il.C_Tax_ID, t.Name, t.Rate, t.IsSalesTax, il.c_activity_id, i.c_activity_id, t.IsCurrencyScheme";
		...
		try
		{
			...
			while (rs.next())
			{
				...
				boolean CurrencyScheme = "Y".equals(rs.getString(7));
				//
				DocTax taxLine = new DocTax(C_Tax_ID, name, rate, 
					taxBaseAmt, amount, salesTax, CurrencyScheme);
				taxLine.setC_Activity_ID(rs.getInt(8));
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		taxUSD = new BigDecimal(0);
		taxSchema = new BigDecimal(0);
		taxtotalSchema = new BigDecimal(0);
			...
			|| getDocumentType().equals(DOCTYPE_ARProForma))
		{
			...
			for (int i = 0; i < m_taxes.length; i++)
			{
				...
				if (amt != null && amt.signum() != 0)
				{
					...
					//Verifica si el impuesto esta marcado para contabilizar en Bs. @rmontano@
					if(m_taxes[i].isCurrencyScheme() && getC_Currency_ID() != as.getC_Currency_ID()){
						taxUSD = taxUSD.add(amt);
						
						if(tl.getAmtAcctCr().equals(new BigDecimal(0))){
							taxtotalSchema = taxtotalSchema.add(tl.getAmtAcctDr());
							taxSchema = taxtotalSchema.negate();
							tl.setAmtSourceDr(tl.getAmtAcctDr());
						}else{
							taxSchema = taxSchema.add(tl.getAmtAcctCr());
							tl.setAmtSourceCr(tl.getAmtAcctCr());
						}
						
						tl.setC_Currency_ID(as.getC_Currency_ID());
					}
			...
			{
				grossAmt = getAmount(Doc.AMTTYPE_Gross).subtract(taxUSD);
				serviceAmt = Env.ZERO;
			}
			...
			//Porcion del impuesto @rmontano@
			if (taxSchema.signum() != 0){
					fact.createLine(null, MAccount.get(getCtx(), receivables_ID),
							as.getC_Currency_ID(), taxSchema, null);
			}
		...
		else if (getDocumentType().equals(DOCTYPE_APInvoice))
		{
			...
			for (int i = 0; i < m_taxes.length; i++)
			{
				...
				//Verifica si el impuesto esta marcado para contabilizar en Bs. @rmontano@
				if(m_taxes[i].isCurrencyScheme() && getC_Currency_ID() != as.getC_Currency_ID()){
					taxUSD = taxUSD.add(m_taxes[i].getAmount());
			...
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				if (!landedCost)
				{
					int order_no = DB.getSQLValue(getTrxName(),"SELECT C_Order_ID FROM C_Invoice WHERE C_Invoice_ID="+get_ID());
					 MAccount expense =null;
					if (order_no > 0)
					     expense = line.getAccount(ProductCost.ACCTTYPE_P_Expense, as);
					else    //Modificacion de Campo de Cuenta adicional para contabilizar gastos directos sin orden de compra
					{
						 expense = line.getAccount(ProductCost.ACCTTYPE_P_Expense2, as);
						 if (expense == null)     //Solo si no consigue la cuenta de gasto directo.
							 expense  = line.getAccount(ProductCost.ACCTTYPE_P_Expense, as);  
					}
					if (line.isItem())
			...
			//Determinar si es a cta por pagar o anticipo de empleado en caso de que sea 
			int payablesReceip = getValidCombination_ID (Doc.ACCTTYPE_C_Receivable, as);
			
		    int tempo = getaccout (get_ID());
		     if (tempo >0)
		    	 payables_ID = tempo;
			...
			if (grossAmt.signum() != 0){
				if (grossAmt.signum() >= 0) {       //Si el monto es positivo va a la cuenta normal
					//Porcion del impuesto @rmontano@
					if (taxSchema.signum() != 0){
					...
					/*if (getC_DocType_ID()==1000064)     //Si es negativo ira a la cuenta por cobrar
					fact.createLine(null, MAccount.get(getCtx(), payablesReceip),
							getC_Currency_ID(), null, grossAmt);
					else*/
						fact.createLine(null, MAccount.get(getCtx(), payables_ID),
								getC_Currency_ID(), null, grossAmt);
	...
	private int getaccout (int invoiceid){
		int account=0;
		MInvoice inv = new MInvoice(getCtx(),invoiceid, null );
		int payment = inv.getC_Payment_ID();
		
		//Service only employee acct
			int prepayment_acct = DB.getSQLValue(getTrxName(),  "SELECT  V_Liability_Services_Acct FROM C_BP_Vendor_Acct WHERE C_BPartner_ID="+inv.getC_BPartner_ID());
		    String employee = DB.getSQLValueString(getTrxName(), "SELECT ISEMPLOYEE FROM C_BPARTNER WHERE C_BPARTNER_ID="+inv.getC_BPartner_ID());
		    
		if (inv.isPayDirect() && employee.equalsIgnoreCase("Y"))   //Esto es solo para empleados
				account=prepayment_acct;
		else
			    account = 0;
		return account;
	}
