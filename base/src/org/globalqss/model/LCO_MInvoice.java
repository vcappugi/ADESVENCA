...
public class LCO_MInvoice extends MInvoice
{
	...
	public int recalcWithholdings() {
		...
		try
		{
			...
			int nodel = DB.executeUpdate(sqldel,null);
			...
			while (rst.next())
			{
                ...
                // include from vcappugi. goal: calculate withholding by line
				// Add tax categories of lines
				if (wrc.isProduct_CIIU()) {
					// look the conf fields
					String sqlwct =
						"SELECT DISTINCT COALESCE (p.LCO_Product_CIIU_ID, COALESCE (c.LCO_ISIC_ID, 0)) "
						+ "  FROM C_InvoiceLine il "
						+ "  LEFT OUTER JOIN M_Product_Acct p ON il.M_Product_ID = p.M_Product_ID "
						+ "  LEFT OUTER JOIN C_Charge c ON (il.C_Charge_ID = c.C_Charge_ID) "
						+ "  WHERE C_Invoice_ID = ? AND il.IsActive='Y'";
					...
					while (rswct.next()) {
						...
						if (wcid > 0) {
							if (i == 0) {
								sqlr.append(" AND LCO_Product_CIIU_ID IN (");
								addedlines = true;
							} else {
								sqlr.append(",");
							}
				...
				if (wrc.isUseMunicipality()) {
				  // Include Municipality in withholding calc
					int municipality = DB.getSQLValue(get_TrxName(), "SELECT C_Municipality_ID FROM C_Invoice WHERE C_Invoice_ID=" + getC_Invoice_ID());
					if (municipality > 0)
						sqlr.append(" AND C_Municipality_ID =" + municipality);
				}
				if (wrc.isUseTaxIdType()){
					int taxidtype = DB.getSQLValue(get_TrxName(),"SELECT (SELECT LCO_TaxIdType_ID from C_BPartner bp1 where bp1.C_BPartner_id=  inv.C_BPartner_ID) "
							+ "from C_Invoice inv where inv.C_Invoice_ID=" + getC_Invoice_ID());
					if (taxidtype>0)
						sqlr.append(" AND LCO_TaxIDType_ID =" + taxidtype);
				}
				//
				// Add tax categories of lines
				if (wrc.isUseProductTaxCategory()) {
					// look the conf fields
					String sqlwct =
						"SELECT DISTINCT COALESCE (t.C_TaxCategory_ID, COALESCE (t.C_TaxCategory_ID, 0)) "
						+ "  FROM C_InvoiceLine il "
						+ "  LEFT OUTER JOIN M_Product p ON (il.M_Product_ID = p.M_Product_ID) "
					...
					if (wc.getBaseType() == null) {
						...
						if (wrc.isUseWithholdingCategory() && wrc.isUseProductTaxCategory()) {
							...
							sqllca = 
								...
								+ "               WHERE il.M_Product_ID = p.M_Product_ID "
//								+ "                 AND p.C_TaxCategory_ID = ?) "
 								+ "                 AND il.C_Tax_ID in (SELECT C_TAX_ID FROM C_TAX WHERE C_TAXCATEGORY_ID=?)) "
								+ "        OR EXISTS ( "
								+ "              SELECT 1 "
								+ "                FROM C_Charge c "
								+ "               WHERE il.C_Charge_ID = c.C_Charge_ID "
//								+ "                 AND c.C_TaxCategory_ID = ?) "
								+ "                 AND il.C_Tax_ID in (SELECT C_TAX_ID FROM C_TAX WHERE C_TAXCATEGORY_ID=?)) "
								+ "       ) ";
						} else if (wrc.isProduct_CIIU()){
							//base : product by CIIU
							sqllca = 
									"SELECT SUM (LineNetAmt) "
									+ "  FROM C_InvoiceLine il "
									+ " LEFT JOIN M_Product_Acct p on p.M_Product_ID = il.M_Product_id"
									+ " LEFT JOIN C_Charge c on c.C_Charge_ID = il.C_Charge_ID"
									+ " WHERE il.IsActive='Y' AND  il.C_Invoice_ID= ?  AND (p.LCO_Product_CIIU_ID = ? or c.LCO_ISIC_ID=? )";
						}else {
							// base = all lines
							sqllca = 
								"SELECT SUM (LineNetAmt) "
						...
						if (wrc.isUseWithholdingCategory() && wrc.isUseProductTaxCategory()) {
							...
							pstmtlca.setInt(2, wr.getLCO_Product_CIIU_ID());
							pstmtlca.setInt(3, wr.getLCO_Product_CIIU_ID());
						}else {
							;  // nothing
						}
	...
	public static boolean updateHeaderWithholding(int C_Invoice_ID, String trxName)
	{
		...
		sql = "UPDATE C_Invoice "
			+ "SET GrandTotal = (TotalLines + (SELECT COALESCE(SUM(TaxAmt),0) from C_InvoiceTax where C_Invoice_ID="+C_Invoice_ID +")) " 
			+ "WHERE C_Invoice_ID=?";
		no = DB.executeUpdate(sql, C_Invoice_ID, trxName);
	...
	//
	/*
	 *Crear linea de invoicetaxt 
	 */
	public int create_invoicetax(){
		String Sql = "SELECT C_Invoice_ID, C_Tax_ID, DateTrx, Taxamt, "
				+" TaxbaseAmt FROM LCO_InvoiceWithholding WHERE C_INVOICE_ID=" + getC_Invoice_ID()
		...
		try {
			...
			while (rs.next ())
			{ 
				MInvoiceTax invtax =  new MInvoiceTax(getCtx(),0,get_TrxName());
				invtax.setC_Invoice_ID(rs.getInt(1));
				invtax.setC_Tax_ID(rs.getInt(2));
				invtax.setTaxAmt(rs.getBigDecimal(4).negate());
				invtax.setTaxBaseAmt(rs.getBigDecimal(5));
				invtax.save();
			}
		} catch (SQLException e) {
