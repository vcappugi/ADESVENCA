...
public class MLCOInvoiceWithholding extends X_LCO_InvoiceWithholding
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if(newRecord){
			if (getLCO_WithholdingRule_ID() > 0) {
				...
				setIsCalcOnPayment( ! wc.isCalcOnInvoice() );
				 ...
				 int idTax = getC_Tax_ID();
		         int idType = getLCO_WithholdingType_ID();
		         String sql= "SELECT LCO_WithHoldingCalc_ID FROM LCO_WithHoldingCalc WHERE C_Tax_ID= " + idTax
		                   + "AND LCO_WithHoldingType_ID = " + idType;
		         int idWC = DB.getSQLValue(get_TrxName(), sql);
		         X_LCO_WithholdingCalc wc = new X_LCO_WithholdingCalc(getCtx(), idWC, get_TrxName());
				...
				/* bloque anterior a Modificacion de rjmontano
				// Fill isCalcOnPayment according to isSOTrx on type
				X_LCO_WithholdingType wt = new X_LCO_WithholdingType (getCtx(), getLCO_WithholdingType_ID(), get_TrxName());
				// set on payment for sales, and on invoice for purchases
		...
		if (getC_AllocationLine_ID() <= 0) {
			...
			//Se coloca la fecha actual por regulacion venezolana
			//MPayment pay = new MInvoice(getCtx(), ,get_TrxName());
			//setDateAcct(inv.getDateAcct());
			//setDateTrx(inv.getDateInvoiced());
			setDateAcct(Env.getContextAsDate(getCtx(), "Date"));
			setDateTrx(Env.getContextAsDate(getCtx(), "Date"));
	...
	{
		int tax_id = getC_Tax_ID();
		int invoice_id = getC_Invoice_ID();
		int borra = DB.executeUpdate("DELETE FROM C_InvoiceTax WHERE C_Tax_ID = " + tax_id + " AND C_Invoice_ID="+ invoice_id );
		if (!success)
			return success;
		return LCO_MInvoice.updateHeaderWithholding(getC_Invoice_ID(), get_TrxName());
	}	//	afterDelete
