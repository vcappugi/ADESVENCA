...
public class LCO_Validator implements ModelValidator
{
	...
	private String accountingForInvoiceWithholdingOnPayment(MAllocationHdr ah) {
		...
		for (int i = 0; i < facts.size(); i++)
		{
			...
			for (int j = 0; j < alloc_lines.length; j++) {
				...
				String sql = 
					  "SELECT i.C_Tax_ID, NVL(SUM(i.TaxBaseAmt),0) AS TaxBaseAmt, NVL(SUM(i.TaxAmt),0) AS TaxAmt, t.Name, t.Rate, t.IsSalesTax, t.IsCurrencyScheme "
					...
					+ "GROUP BY i.C_Tax_ID, t.Name, t.Rate, t.IsSalesTax, t.IsCurrencyScheme";
				PreparedStatement pstmt = null;
				ResultSet rs = null;
				try
				{
					...
					while (rs.next()) {
						...
						boolean isCSchema = rs.getString(7).equals("Y") ? true : false;
						
						DocTax taxLine = new DocTax(tax_ID, name, rate, 
								taxBaseAmt, amount, salesTax, isCSchema);
						...
						if (amount != null && amount.signum() != 0)
						{
							...
							if (invoice.isSOTrx()) {
								...
								if(taxLine.isCurrencyScheme()){
									...
									tl.setC_Currency_ID(as.getC_Currency_ID());
									amount = tl.getAmtAcctCr();
								}
								
							}
							if (tl != null) {
								tl.setC_Tax_ID(taxLine.getC_Tax_ID());
								tl.setC_Activity_ID(alloc_line.getC_Activity_ID());
								tl.setC_Project_ID(alloc_line.getC_Project_ID());
							}
				...
				if (Env.ZERO.compareTo(tottax) != 0)
				{
					...
					if (!foundflwriteoff) {
						...
						if (fl != null) {
							...
							fl.setC_Activity_ID(alloc_line.getC_Activity_ID());
							fl.setC_Project_ID(alloc_line.getC_Project_ID());
						}
					}
	...
	private String completeInvoiceWithholding(MInvoice inv) {
		...
		//Delete withholding not payment
				String upd_proc2 =
					"DELETE C_InvoiceTax "
					 + " WHERE C_Invoice_ID = ? AND C_Tax_ID in (Select C_Tax_ID from LCO_InvoiceWithholding WHERE "
					 +		"IsCalcOnPayment = 'Y' AND C_Invoice_ID="+inv.getC_Invoice_ID() + ")";
				int noupdproc2 = DB.executeUpdate(upd_proc2, inv.getC_Invoice_ID(),  inv.get_TrxName());
				if (noupdproc2 == -1)
					return "Error updating processed on invoice withholding";
				else{
					LCO_MInvoice lcoinv = new LCO_MInvoice(inv.getCtx(),inv.getC_Invoice_ID() ,inv.get_TrxName());
					lcoinv.updateHeaderWithholding(inv.getC_Invoice_ID(), inv.get_TrxName());
